...
1. 유지보수가 좋아짐
2. 여러 곳에서 같은 로직을 사용하고 있다고 확신할 수 있음
3. 이름이 붙어있기 때문에 코드의 정체를 분명하게 이해할 수 있음
...
매개변수가 하나라면 소괄호 조차도 생략할 수 있지만, 하나도 없을 경우엔 소괄호를 비워놓고 이때는 생략할 수 없다고 한다.
...
콜백 함수는 매개변수로 함수 객체를 전달해서 호출 함수 내에서 매개변수 함수를 실행하는 것이라는데...

간단하게 callback이라는 매개변수에 함수 자체가 인자로 전달되는 것이라고 생각하자
...
sum을 호출하면 2와 3을 더한 값이 num에 들어가고 이 num의 값을 콜백 함수인 result의 인자로 사용하게 되는 것이다.

이후에 result가 실행되어 "결과는 5 입니다." 라는 문장이 출력된다.
